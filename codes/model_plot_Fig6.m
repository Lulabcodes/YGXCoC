...
for i = 1:length(RATIO_G)
    ...
    [T,Y] = ode15s(@(t,y)Kinetic_Equation(t,y,Para_compo),[0 150],[ini_Xyl;ini_Glu;0;OD_X;OD_G;0;0;1;1],opts);
...
plot(RATIO_G, Em/(ini_Glu+ini_Xyl),'k','LineWidth',2);
...
for i = 1:length(Tset)
    DT = Tset(i);
    [t1,y1] = ode15s(@(t,y)Kinetic_Equation(t,y,Para_tempo),[0 DT],[ini_Xyl;ini_Glu;0;9;0;0;0;1;1],opts);
    Y0 = y1(end,:);
    Y0(5) = 9;
...
for i = 1:length(Tset)
    DT = Tset(i);    
    [t1,y1] = ode15s(@(t,y)Kinetic_Equation(t,y,Para_tempo),[0 DT],[ini_Xyl;ini_Glu;0;0;9;0;0;1;1],opts);
    Y0 = y1(end,:);
    Y0(4) = 9;
    [t2,y2] = ode15s(@(t,y)Kinetic_Equation(t,y,Para_tempo),[t1(end), 120], Y0,opts);
    T = [t1(1:end-1);t2];
...
title('Fig.6b');
plot(Tset, Emp/(ini_Glu+ini_Xyl),'k','LineWidth',2);
plot(-Tset, Emn/(ini_Glu+ini_Xyl), 'k-','LineWidth',2);
