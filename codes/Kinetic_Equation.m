...
m_x = Para_set(14);  m_g = Para_set(14);  K_mx = 1e-6;  K_mg = 1e-6;       % Flux to Maintenance 
...
J_x   = Hill(a_x,  K_x,  X) * 1/(1+k_x*E) * Cx/(Cx+K_C);                   % xylose consumption rate
...
J_mx  = Hill(m_x, K_mx, Rx);                                                % Flux rate to maintenance (YX)
J_mg  = Hill(m_g, K_mg, Rg);                                                % Flux rate to maintenance (YX)
J_ecx = Hill(a_ecx,K_ecx,E) * 1/(1+k_ecx*X);                               % Ethanol reassimilation rate (YX)
J_ecg = Hill(a_ecg,K_ecg,E) * 1/(1+k_ecg*G);                               % Ethanol reassimilation rate (YG)
...
if Nx == 0                                                                 % Keep intracellular concentrations unchanged until the
    ...
    dRxdt = b_x * J_x + b_ecx * J_ecx - J_nx - J_epx - J_mx - g_x * J_nx * Rx;
...
dydt = [dXdt;dGdt;dEdt;dNxdt;dNgdt;dRxdt;dRgdt;dOdt;dCxdt];
